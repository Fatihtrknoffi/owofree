...
var kanal = 925705476484788234
var sahip = 733743801373294622

client.on("ready",()=>{
	console.log("GelecekTeam => OWO PARA KASMA BOTU AKTIF");
});
...
client.on("message",message=>{
	if(message.channel.id==kanal){
		if(message.author.id=="408785106942164992"){ // BURAYI ELLEMEYIN
			if(message.content.includes("Beep Boop") || message.content.includes("Please DM me") || message.content.includes("Are you a real human?")){
				db.set("owodg","1");
				message.channel.send("GelecekTeam owo para kasma sistemi durduruldu.");
			};
		};
	};
	...
	if(message.author.id == sahip){
		if(message.content == "!aç"){
			db.set("owodg","0");
			message.channel.send("GelecekTeam owo para kasma sistemi açıldı.");
		};
	};
	...
	if(message.author.id == sahip){
		if(message.content == "!kapat"){
			db.set("owodg","1");
			message.channel.send("GelecekTeam owo para kasma sistemi kapatıldı.");
		};
	};
});

client.login("NzMzNzQzODAxMzczMjk0NjIy.YcxAMw.q9CGayX6CtBV0saFa1SiNox1Mbo");